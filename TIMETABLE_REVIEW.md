...
## Discrepancy review — 22 September 2026

The application has **no live vehicle-position or delay feed**. Synchronizing an animation to the Bangkok clock cannot make it match actual trains. The official BTS route map exposes a Train Time Arrival view; this update links users to that view instead of presenting the simulation as real observations. A supported third-party feed and its reuse terms have not been established. No API credentials were copied and no undocumented encrypted arrival API was integrated.

SRTET's current terminal tables were downloaded again on 22 September and the departures matched the embedded lists. A separate model discrepancy was corrected: Dark Red previously took approximately 22.12 minutes in both directions instead of the operator's approximate 23 minutes outbound and 24 minutes returning from Rangsit; Light Red took approximately 14.61 instead of 16 minutes. Motion timelines now reach the terminal at those published durations, preserving bounded speeds and station stops. Intermediate station timings remain estimates. Sources: https://www.srtet.co.th/th/fare-timetable , https://www.srtet.co.th/th/fare-timetable?s=12 , https://www.srtet.co.th/th/fare-timetable?s=0 .

The UTC+7 calculation was checked across local midnight and weekend transitions. It reads the device clock; no independent server-time synchronization is claimed. The collapsed time-control button now retains “Not live”, and BTS station details link to https://www.bts.co.th/routemap.html . This is a correction to model timing and presentation, **not completion of live tracking**. A specific user-reported station/time discrepancy still requires the affected line, direction, station and observed time.
